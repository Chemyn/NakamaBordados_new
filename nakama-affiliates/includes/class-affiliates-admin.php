...
final class Nakama_Affiliates_Admin {
	...
	private static function render_settings() {
		...
		$legacy_user_id = absint( self::query_value( 'legacy_user_id' ) );
		$legacy_preview = $legacy_user_id ? Nakama_Affiliates_Legacy_Migration::preview( $legacy_user_id ) : null;
		?><section aria-labelledby="nka-settings-title"><div class="nka-section-heading"><div><p class="nka-kicker">Reglas vigentes</p><h2 id="nka-settings-title">Configuración</h2></div></div><div class="nka-settings-grid"><article><h3>Descuento</h3><strong>Máximo 10%</strong><p>Exclusivo; sustituye otras promociones.</p></article><article><h3>Comisión</h3><strong>10%</strong><p>Sobre subtotal elegible antes del descuento; excluye envío y se revierte con devoluciones.</p></article><article><h3>Fiscal</h3><strong>Captura manual</strong><p>Sin facturación automática ni cálculo de ISR/IVA en esta versión.</p></article></div><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="nka-form-grid"><?php self::mutation_fields( 'save_program_settings' ); ?><label class="nka-wide">IDs de categorías restringidas<input name="restricted_category_ids" value="<?php echo esc_attr( $category_ids ); ?>" placeholder="123, 456" /><small>Drops y Edición especial. Los afiliados VIP sí podrán ver estas categorías.</small></label><label class="nka-wide">Cuentas oficiales de Nakama<textarea name="official_accounts" placeholder="@nakamabordados&#10;@otra_cuenta"><?php echo esc_textarea( $accounts ); ?></textarea><small>Una cuenta por línea. Administración verificará manualmente las etiquetas en Reels e Historias.</small></label><button class="button button-primary" type="submit">Guardar configuración</button></form></section>
		<section aria-labelledby="nka-legacy-title"><div class="nka-section-heading"><div><p class="nka-kicker">Migración controlada</p><h2 id="nka-legacy-title">Comisiones del prototipo anterior</h2></div></div><div class="nka-info-box"><strong>Primero previsualiza.</strong> Solo se leen metadatos exactos <code>apoyo_creador_AÑO_MES</code>. La importación conserva el origen, no toma valores demostrativos de la interfaz y no inventa ventas ni progreso de prendas.</div><form method="get" action="<?php echo esc_url( admin_url( 'admin.php' ) ); ?>" class="nka-filter"><input type="hidden" name="page" value="<?php echo esc_attr( self::SLUG ); ?>" /><input type="hidden" name="tab" value="settings" /><label>ID del usuario<input name="legacy_user_id" type="number" min="1" value="<?php echo esc_attr( $legacy_user_id ?: '' ); ?>" required /></label><button class="button" type="submit">Previsualizar metadatos</button></form>
		<?php if ( is_array( $legacy_preview ) && ! empty( $legacy_preview['success'] ) ) : ?>
			<table class="widefat striped"><thead><tr><th>Periodo</th><th>Clave de origen</th><th>Comisión histórica</th><th>Estado</th></tr></thead><tbody><?php foreach ( $legacy_preview['items'] as $item ) : ?><tr><td><?php echo esc_html( $item['period'] ); ?></td><td><code><?php echo esc_html( $item['meta_key'] ); ?></code></td><td><?php echo esc_html( self::money( $item['commission_mxn'] ) ); ?></td><td><?php echo esc_html( 'importable' === $item['status'] ? 'Lista para importar' : ( 'already_imported' === $item['status'] ? 'Ya importada' : 'Periodo ya cerrado' ) ); ?></td></tr><?php endforeach; ?><?php if ( empty( $legacy_preview['items'] ) ) : ?><tr><td colspan="4">No se encontraron importes históricos válidos para este usuario.</td></tr><?php endif; ?></tbody></table>
			<?php if ( ! empty( $legacy_preview['importable'] ) ) : ?><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="nka-form-grid"><?php self::mutation_fields( 'migrate_legacy' ); ?><input type="hidden" name="legacy_user_id" value="<?php echo esc_attr( $legacy_user_id ); ?>" /><label class="nka-check nka-wide"><input name="confirmed" type="checkbox" value="1" required /> Confirmo que revisé los periodos y montos. Se crearán movimientos históricos de comisión sin base de ventas.</label><button class="button button-primary" type="submit" data-nakama-confirm="La importación conservará los metadatos de origen y no podrá duplicarse. ¿Continuar?">Importar comisiones revisadas</button></form><?php endif; ?>
		<?php elseif ( is_array( $legacy_preview ) ) : ?><div class="notice notice-warning inline"><p>No se pudo preparar la vista previa. Verifica que el usuario ya tenga un perfil de afiliado.</p></div><?php endif; ?></section><?php
	}
	...
	public static function process_action( $action, array $data, $actor_user_id ) {
		switch ( (string) $action ) {
			...
			case 'migrate_legacy':
				return Nakama_Affiliates_Legacy_Migration::migrate( absint( $data['legacy_user_id'] ?? 0 ), ! empty( $data['confirmed'] ), (int) $actor_user_id );
	...
	private static function action_tab( $action ) {
		...
		if ( in_array( $action, array( 'save_program_settings', 'migrate_legacy' ), true ) ) return 'settings';
